...
def generate_sample_trajectories_grid(adata, model, epoch, top_p, top_k, temperature, output_dir, accelerator):
    num_cells = len(adata)
    ...
    result_dir = os.path.join(output_dir, f"epoch_{epoch}")
    ...
    plot(
        ...
        save=os.path.join(result_dir, "trajectory_plot.png")
    )
    ...
    accelerator.save_state(result_dir)
    print(f"Checkpoint saved in: {result_dir}")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if isinstance(checkpointing_steps, int) and completed_steps % checkpointing_steps == 0 and accelerator.sync_gradients:
                ...
                clean_old_checkpoints(args.output_dir, n=2)
            ...
            if completed_steps % evaluation_interval == 0:
                ...
                generate_sample_trajectories_grid(adata,
                                                  model,
                                                  epoch,
                                                  top_p_values,
                                                  top_k_values,
